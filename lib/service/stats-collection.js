...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'flora-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('flora').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('flora')[service]);
